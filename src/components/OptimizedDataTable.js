...
const OptimizedTableCell = memo(({ 
  ...
  return (
    <div className={`${cellStyling} group relative min-h-[60px] flex items-center`}>
      <div className="flex-1 truncate" title={cell?.cleaned || ''}>
    ...
    <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
      ...
      <div className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
        <div className="grid grid-cols-12 gap-0">
          <div className="col-span-1 px-3 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">
            Row
          </div>
          {columnHeaders.map((header, index) => (
            <div key={index} className="col-span-1 px-3 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">
              {header}
            </div>
          ))}
        </div>
        ...
        <div style={{ height: totalHeight, position: 'relative' }}>
          <div style={{ transform: `translateY(${offsetY}px)` }}>
            {visibleDataWithIndices.map(({ rowData, actualIndex }) => (
              <div key={actualIndex} className="grid grid-cols-12 gap-0 hover:bg-gray-50">
                <div className="col-span-1 px-3 py-2 text-sm text-gray-500 border-b border-gray-200 bg-gray-50">
                  {actualIndex + 1}
                </div>
                {rowData.map((cell, colIndex) => (
                  <OptimizedTableCell
                    key={`${actualIndex}-${colIndex}`}
                    cell={cell}
                    colIndex={colIndex}
